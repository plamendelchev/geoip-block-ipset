...
var (
	...
	errIpsetNotSupported = errors.New(
...
func (s *IPSet) createHashSet(name string) error {
	out, err := exec.Command(ipsetPath, "create", name, s.HashType, "family", s.HashFamily, "hashsize", strconv.Itoa(s.HashSize),
		"maxelem", strconv.Itoa(s.MaxElem), "timeout", strconv.Itoa(s.Timeout), "-exist").
		CombinedOutput()
	if err != nil {
		return fmt.Errorf(
			"error creating ipset %s with type %s: %v (%s)",
			name,
			s.HashType,
			err,
			out,
...
func (s *IPSet) Refresh(entries []string) error {
	// Create temp set
	tempName := fmt.Sprintf("%s-temp", s.Name)
	err := s.createHashSet(tempName)
	if err != nil {
		return err
	}
	// Build list to restore
	var buffer bytes.Buffer
	for _, entry := range entries {
		rule := fmt.Sprintf("add %s %s timeout %d", tempName, entry, s.Timeout)
		buffer.WriteString(rule)
		buffer.WriteString("\n") // Adding new line between rules
	}
	// Use "restore" to import new rules to set
	cmd := exec.Command(ipsetPath, "restore")
	cmd.Stdin = &buffer
	out, err := cmd.CombinedOutput()
	if err != nil {
		log.Errorf("error restoring set %s: %+v %+v", tempName, err, out)
	}
	// Swap sets
...
func (s *IPSet) Add(entry string, timeout int) error {
	out, err := exec.Command(ipsetPath, "add", s.Name, entry, "timeout", strconv.Itoa(timeout), "-exist").
		CombinedOutput()
	if err != nil {
...
func (s *IPSet) AddOption(entry string, option string, timeout int) error {
	out, err := exec.Command(ipsetPath, "add", s.Name, entry, option, "timeout", strconv.Itoa(timeout), "-exist").
		CombinedOutput()
	if err != nil {
